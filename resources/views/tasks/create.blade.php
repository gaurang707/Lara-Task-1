                ...
                <form action="{{ route('tasks.store') }}" method="post">
                    ...
                    <div class="mb-6">
                        <label for="name" class="block mb-2.5 text-sm font-medium text-heading">Prject
                            name</label>
                        <select name="project_id" id="project_id_select">
                    ...
                    <div class="mb-6">
                        ...
                        <select name="status" id="">
                            @foreach ($statuses as $statusval)
                                <option value="{{ $statusval }}" @if (old('status') == $statusval) selected @endif>
                                    {{ $statusval }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <select name="assigned_id">
                            ...
                            @foreach ($assignees as $assignee)
                                <option value="{{ $assignee->id }}" @if (old('assigned_id') == $assignee->id) selected @endif>
    ...
    <script>
        $(function() {
            ...
            $('#project_id_select').on('change', function() {
                ...
                if (selectedValue) {
                    $.ajax({
                        url: "{{ route('tasks.getdeadline') }}", // Use Laravel route helper
                        method: 'POST',
                        data: {
                        ...
                        success: function(response) {                           
                            ...
                            $('#responseContainer').html('<p>Deadline: ' + response.message +
                                '</p>');
                        },
                        error: function(xhr, status, error) {
                            console.error(xhr.responseText);
                            $('#responseContainer').html('<p>Error: ' + xhr.responseText +
                                '</p>');
                        }
                    });
                } else {
